  ...
  for line in lines:
      ...
      if line.startswith("Timestamp:"):
          ...
          try:
              ...
              c.execute("INSERT INTO changes (timestamp, change_type, row_number, old_value, new_value) VALUES (?, ?, ?, ?, ?)",
          ...
          try:
              ...
              old_value, new_value = parts[1].split(" -> ")
              c.execute("INSERT INTO changes (timestamp, change_type, row_number, old_value, new_value) VALUES (?, ?, ?, ?, ?)",
...
def get_recent_changes(conn, limit=10):
    c = conn.cursor()
    c.execute("SELECT * FROM changes ORDER BY id DESC LIMIT ?", (limit,))
    return c.fetchall()